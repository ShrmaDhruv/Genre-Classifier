...
def audio_to_spectrogram(audio_path, output_path, sr=22050, n_mels=128, fmax=8000):
    """
...
def audio_to_spectrogram_gpu(audio_path, output_path, sr=22050, n_mels=128, fmax=8000):
    ...
    # Create figure
    plt.figure(figsize=(10, 4))
    librosa.display.specshow(mel_spec_db_np, sr=sr, x_axis='time', y_axis='mel', fmax=fmax)
    plt.colorbar(format='%+2.0f dB')
    plt.title(Path(audio_path).stem)
    plt.tight_layout()
